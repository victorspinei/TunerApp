...
class Tuner {
  public:
    Tuner(float reference_frequency): reference_frequency(reference_frequency) {
        initialize_notes();
    };
    ...
    float note_frequency(int semitones) {
    ...
    TuningResult result(float frequency) {
  ...
  private:
    ...
    const int A4_index = 48;
    float reference_frequency;

    void initialize_notes() {
        for (int i = 0; i < note_names.size(); i++) {
            note_frequencies.push_back(note_frequency(i - A4_index));
        }
    }
};
